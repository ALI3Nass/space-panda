...
from services.google_sheets_service import fetch_candidates
from services.google_drive_service import download_cv
from services.cv_processing_service import process_cv
import os
...
def shortlist_candidates():
    ...
    for job_id in shortlisted:
        shortlisted[job_id] = sorted(shortlisted[job_id], key=lambda x: x['score'], reverse=True)[:5]

        # Rename and save shortlisted CVs
        for index, candidate in enumerate(shortlisted[job_id]):
            new_file_name = f"{job_id}_{index + 1}.pdf"
            os.rename(f"shortlisted_cvs/{candidate['name']}.pdf", f"shortlisted_cvs/{new_file_name}")
